...
public class ChessRuleEngine implements RuleEngine {
    ...
    public boolean isLegalMove(Board board, Move m, ChessColor side) {
        ...
        // Pieces.Pawn-specific capture vs forward verification:
     ...
     * - Pieces.Pawn forward double-step: intermediate must be empty
        ...
        for (int r = 0; r < dims.rows(); r++) {
            for (int c = 0; c < dims.cols(); c++) {
                ...
                for (Move m : candidates) {
                    ...
                    // Pieces.Pawn diagonal must be capture-like; but since this is attack detection we count diagonals as attacks
                    return true;
                }
            }
        }
